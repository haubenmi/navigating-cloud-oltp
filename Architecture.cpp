...
Primary::Primary(const Parameter& p, const Node& n, bool rbpex) : p{p}, n{n}, usesBufferPoolExtension{rbpex} {
  if (p.lookupZipf != 0.0) {
     ...
     auto datasetGB = p.datasetSize / 1024 / 1024 / 1024;
     ...
     probCacheHitVal = (1.0 * dataInCache()) / p.datasetSize;
     probFirstCacheHitVal = (1.0 * dataInFirstCache()) / p.datasetSize;
     probSecondCacheHitVal = (1.0 * dataInSecondCache()) / p.datasetSize;
  }
                                                                              }
//--------------------------------------------------------------------------------
...
Rate Primary::getCacheHitOps(Rate alreadyUsed) const {
      if (!usesBufferPoolExtension) return n.cpu.getOps() - alreadyUsed;
      ...
      return vmin(n.cpu.getOps() - alreadyUsed, remainingWrites / probSecondCacheHit(), remainingReads / probSecondCacheHit());
   }
//--------------------------------------------------------------------------------
