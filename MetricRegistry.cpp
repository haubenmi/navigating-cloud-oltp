...
void MetricRegistry::print(ostream& out) {
   for (size_t i = 0; i < architectures.size(); ++i) {
      auto& a = architectures[i];
      printArch(out, *a, i);
   }
}
//--------------------------------------------------------------------------------
void MetricRegistry::sort(string_view col) {
   ...
   std::sort(architectures.begin(), architectures.end(), [&](const auto& a, const auto& b) {
...
void MetricRegistry::filter() {
  std::erase_if(architectures, [&](auto a) { return std::any_of(metrics.begin(), metrics.end(), [&](auto& m) { return m->shouldExclude(*a); }); });
}
//--------------------------------------------------------------------------------
void MetricRegistry::trunc(unsigned minPerArch) {
   // Relative order is preserved, so we can do this after sorting

   unordered_map<ArchType, unsigned> counts;
   vector<const Architecture*> result;
   for (auto& a : architectures) {
      if (++counts[a->getType()] <= minPerArch) {
         result.push_back(a);
      }
   }
   architectures.swap(result);
}
//--------------------------------------------------------------------------------
