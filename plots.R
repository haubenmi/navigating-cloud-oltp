...
options(sqldf.RPostgreSQL.user = "ruser",
  sqldf.RPostgreSQL.password = "rpass",
...
#   updates <- requiredOps * (percentUpdates / 100)
#   lookups <- requiredOps - updates
   cloud_calc_cmd <- paste(paste0(baseDir, "./cloud_calc"),
                    "--vantage-csv",
                    paste0(baseDir, "./vantage.csv"),
...
architectureColors <- c("classic" = reduceSat(rgb(174,199,232, maxColorValue = 255)),
                        "hadr" = reduceSat(rgb(148,103,189, maxColorValue = 255)),
                        "rbd" = reduceSat(rgb(31,119,180, maxColorValue = 255)),
                        "aurora" = reduceSat(rgb(44,160,44, maxColorValue = 255)),
                        "socrates" = reduceSat(rgb(188,189,34, maxColorValue = 255)),
                        "inmem" = reduceSat(rgb(255,127,14, maxColorValue = 255))
                        )
...
architectureColors2 <- c("Classic" = reduceSat(rgb(174,199,232, maxColorValue = 255)),
                        "HADR" = reduceSat(rgb(148,103,189, maxColorValue = 255)),
                        "RBD" = reduceSat(rgb(31,119,180, maxColorValue = 255)),
                        "Aurora-like" = reduceSat(rgb(44,160,44, maxColorValue = 255)),
                        "Socrates-like" = reduceSat(rgb(188,189,34, maxColorValue = 255)),
                        "In-Memory" = reduceSat(rgb(255,127,14, maxColorValue = 255))
                        )
datasetLabels = c(
    "1073741824" =      "1GB  ",
    "10737418240" =     "10GB ",
...
durabilityLevels <- c(4)#,4,7,11)
interAZ <- c(FALSE)
noGroupCommit <- FALSE
finalData <- runAllCalcs(opsVector,skewVector, percentUpdatesVector,latencyLevels,datasetSizes, durabilityLevels, interAZ, noGroupCommit)

readsForDataSize <- sqldf(paste0('WITH combinations AS (SELECT ops,"DataSize","percentUpdates","requiredDurability","requiredLatency" FROM "finalData" GROUP BY ops,"DataSize","percentUpdates","requiredDurability", "requiredLatency"),
bestIds AS (
...
SELECT (CASE WHEN fd."numSec" > 0 THEN (fd."numSec"+1)::text || \'×\' ELSE \'\' END) || replace(fd."Primary",\'-rbpex\',\'\') || \'\n$\' || (case when fd."TotalPrice">=9.9 THEN (fd."TotalPrice"::int)::text when fd."TotalPrice">=0.95 THEN round(fd."TotalPrice"::numeric(10,1),1)::text ELSE round(fd."TotalPrice"::numeric(10,2),2)::text END) as "NamePrice",
    ...
    fd."DataSize", fd.ops, fd."percentUpdates" FROM "finalData" fd, bestIds bi WHERE bi.ops=fd.ops AND bi."percentUpdates"=fd."percentUpdates" AND bi."DataSize"=fd."DataSize" AND bi."requiredLatency"=fd."requiredLatency" AND bi."requiredDurability"=fd."requiredDurability" AND fd.id=bi.bestId;'))

readsForDataSize$myPercent = ordered(readsForDataSize$percentUpdates,levels=c('0','10','30','100'),labels=c('100% Lookups','90% Lookups; 10% Updates','30% Updates; 4x9\'s Durability','100% Updates'))
plot1 <- ggplot(readsForDataSize, aes(x=factor(DataSize),y=factor(ops),fill=Type)) +
    ...
    geom_text(mapping=aes(label=NamePrice), size=3.2) +
...
ggsave(plot = plot1, filename = "typical.pdf", device = cairo_pdf, width = 3.8, height = 3.9)
########################################


# Compare architecture to best instance
bestPriceData <- sqldf(paste0('
WITH combinationsWithType AS (SELECT ops,"DataSize","percentUpdates","Type","requiredDurability","requiredLatency" FROM "finalData" GROUP BY 1,2,3,4,5,6),
combinations AS (
...
SELECT fd."Primary",
       ...
       bp.bestPrice
       FROM "finalData" fd, bestPriceForType bpft, bestPrice bp
...
scale_fill_gradientn(colours=c(rgb(48,147,67,maxColorValue=255),rgb(255,221,113,maxColorValue=255),rgb(216,37,38,maxColorValue=255)), values=c(0,0.01,1)) +
    geom_tile(color="black") +
...
ggsave(plot = plot, filename = "bestprice.pdf", device = cairo_pdf, width = 7.5, height = 2.81)
########################################
...
interAZ <- c(FALSE)
noGroupCommit <- FALSE
finalDataDura4 <- runAllCalcs(opsVector,skewVector, percentUpdatesVector,latencyLevels,datasetSizes, c(1), interAZ, noGroupCommit)

readsForDataSize <- sqldf(paste0('WITH combinations AS (SELECT "Type",ops,"DataSize","percentUpdates","requiredDurability","requiredLatency" FROM "finalDataDura4" GROUP BY 1,2,3,4,5,6),
bestIds AS (
  SELECT cd.*,(SELECT fd.id
               FROM "finalDataDura4" fd
...
FROM "finalDataDura4" fd, bestIds bi
...
readsForDataSize$myType = factor(readsForDataSize$Type, levels=archLevels, labels=archNames)
plot27 <- ggplot(readsForDataSize, aes(x=lat/1000,y=TotalPrice,group=myType,shape=myType,color=myType)) +
...
#    expand_limits(y=0,x=0) +
    scale_color_manual(values = architectureColors2) +
...
ggsave(plot = plot27, filename = "latencies4.pdf", device = cairo_pdf, width = 3.8, height = 2.3)
...
skewData$myType = factor(skewData$Type, levels=archLevels, labels=archNames)
skewData$myLabel = factor(skewData$label, levels=archLevels, labels=archNames)
plot <- ggplot(skewData, aes(x=lookupZipf,y=TotalPrice,group=myType,shape=myType,color=myType,label=myLabel)) +
...
readsForDataSize$myPercent = ordered(readsForDataSize$percentUpdates,levels=c('0','10','30','100'),labels=c('100% Lookups','90% Lookups; 10% Updates','70% Lookups; 30% Updates','100% Updates'))
plot27 <- ggplot(readsForDataSize, aes(x=factor(DataSize),y=factor(ops),fill=Type)) +
    scale_fill_manual(values = architectureColors, label=archNames) +
    geom_tile(color="black") +
    geom_text(mapping=aes(label=flett), size=3, alpha=0.25, position=position_nudge(x=-0.38, y=-0.30)) +
    geom_text(mapping=aes(label=NamePrice), size=3.2) +
    scale_x_discrete(name = "Dataset Size", label = datasetLabels) +
    scale_y_discrete(name = "Operations / sec", labels=scientific_10) +
    facet_grid(rows=vars(dura),cols=vars(myPercent)) +
    theme_bw() +
  theme(
    axis.text.x=element_text(size=12),
    axis.text.y=element_text(size=14),
...
ggsave(plot = plot27, filename = "areaplot_relaxed.pdf", device = cairo_pdf, width = 12.2, height = 9.8)

updateRatioData <- sqldf(paste0('WITH
combinations AS (SELECT ops,"DataSize","requiredDurability","percentUpdates" FROM "finalData" GROUP BY 1,2,3,4),
bestIds AS (SELECT cd.*,(SELECT fd.id FROM "finalData" fd WHERE fd.ops=cd.ops AND fd."DataSize"=cd."DataSize" AND fd."requiredDurability"=cd."requiredDurability" AND fd."percentUpdates"=cd."percentUpdates" ORDER BY fd."TotalPrice" ASC, fd."Durability" DESC, fd."OpLatency" ASC, "numSec" ASC, fd."Type" ASC, fd.id ASC LIMIT 1) as bestId FROM combinations cd),
basePrice AS (SELECT bi.*,fd."TotalPrice" as price FROM bestIds bi, "finalData" fd WHERE bi."percentUpdates"=30 AND bi."requiredDurability"=4 AND bi.ops=fd.ops AND bi."DataSize"=fd."DataSize" AND bi.bestId=fd.id AND bi."requiredDurability"=fd."requiredDurability" AND bi."percentUpdates"=fd."percentUpdates"),
...
FROM basePrice ba, bestIds bi, "finalData" fd WHERE bi.ops=fd.ops AND bi."DataSize"=fd."DataSize" AND bi."requiredDurability"=fd."requiredDurability" AND bi.bestId=fd.id
AND ba.ops=fd.ops AND ba."DataSize"=fd."DataSize" AND bi."percentUpdates"=fd."percentUpdates" AND fd."requiredDurability"=4)
SELECT * FROM result;'))
...
updateRatioData$myDura <- ordered(updateRatioData$dura, levels=c(0,1,4,7,11), labels=c("No Durability Required","1×9's Durability","4×9's Durability","7×9's Durability", "11×9's Durability")) # Multi-AZ Deploy. +
plot_dura_ratio <- ggplot(updateRatioData, aes(x=factor(DataSize),y=factor(ops),fill=priceRatio)) +
...
ggsave(plot = plot_dura_ratio, filename = "update_ratio_price.pdf", device = cairo_pdf, width = 6.3, height = 1.7) # 4.4 width for one
...
datasetSizes <- c(10,100,1000,10000,100000)
latencyLevels <- c(10000000)#c(1000,10000,100000,1000000)
durabilityLevels <- c(1,4,11)#,4,7,11)
interAZ <- c(FALSE)
noGroupCommit <- FALSE
...
########################################
durabilityData$myDura <- ordered(durabilityData$dura, levels=c(0,1,4,7,11), labels=c("No Durability Required","1×9's Durability","4×9's Durability","7×9's Durability", "11×9's Durability")) # Multi-AZ Deploy. +
plot <- ggplot(durabilityData, aes(x=factor(DataSize),y=factor(ops),fill=Type)) +
    ...
    geom_tile(color="black") +
    geom_text(mapping=aes(label=flett), size=3, alpha=0.25, position=position_nudge(x=-0.38, y=-0.00)) +
...
ggsave(plot = plot, filename = "durabilities.pdf", device = cairo_pdf, width = 7.7, height = 2.95) # 4.4 width for one
########################################
...
basePrice AS (SELECT bi.*,fd."TotalPrice" as price FROM bestIds bi, "durabilityRawData" fd WHERE bi."requiredDurability"=4 AND bi.ops=fd.ops AND bi."DataSize"=fd."DataSize" AND bi.bestId=fd.id AND bi."requiredDurability"=fd."requiredDurability"),
...
AND ba.ops=fd.ops AND ba."DataSize"=fd."DataSize" AND fd."requiredDurability" != 4)
SELECT * FROM result;'))
head(durabilityRatioData)

durabilityRatioData$myDura <- ordered(durabilityRatioData$dura, levels=c(0,1,4,7,11), labels=c("No Durability Required","1×9's vs. 4×9's","4×9's Durability","7×9's Durability", "4×9's vs. 11×9's")) # Multi-AZ Deploy. +
...
ggsave(plot = plot_dura_ratio, filename = "durabilities_ratio.pdf", device = cairo_pdf, width = 3.2, height = 1.88) # 4.4 width for one
########################################


########################################
opsVector <- c(1000,10000,100000,1000000)
skewVector <- c(0.0)
percentUpdatesVector <- c(30)#,30,100)
datasetSizes <- c(10,100,1000,10000)
latencyLevels <- c(10000000)#c(1000,10000,100000,1000000)
durabilityLevels <- c(1)#,4,7,11)
interAZ <- c(FALSE)
noGroupCommit <- FALSE
costCompRawData <- runAllCalcs(opsVector,skewVector, percentUpdatesVector,latencyLevels,datasetSizes, durabilityLevels, interAZ, noGroupCommit)

costDistData <- sqldf(paste0('WITH combinations AS (SELECT ops,"DataSize","Type" FROM "costCompRawData" GROUP BY 1,2,3),
vars(x) AS (SELECT * FROM (VALUES (\'prim\'),(\'sec\'),(\'page\'),(\'log\'),(\'network\'),(\'ebs\'))),
bestIds AS (SELECT cd.*,(SELECT fd.id FROM "costCompRawData" fd WHERE fd.ops=cd.ops AND fd."Type"=cd."Type" AND fd."DataSize"=cd."DataSize" ORDER BY fd."TotalPrice" ASC, "numSec" ASC, fd."Durability" DESC, fd."Type" DESC, fd.id ASC LIMIT 1) as bestId FROM combinations cd),
result AS (
SELECT
    fd."Type",
    upper(substr(fd."Type",0,2)) as flett,
    ...
    CASE v.x WHEN \'prim\' THEN replace(fd."Primary",\'-rbpex\',\'\')
             WHEN \'sec\' THEN (CASE WHEN fd."numSec" > 0 THEN fd."numSec" || \'×\' ELSE \'\' END) || replace(fd."Primary",\'-rbpex\',\'\')
             ELSE \'\' END as mylabel,
    CASE v.x WHEN \'prim\' THEN fd."PrimPrice"
             WHEN \'sec\' THEN fd."SecPrice"
             WHEN \'page\' THEN fd."PageSvcPrice"
             WHEN \'ebs\' THEN fd."EBSPrice"
             WHEN \'log\' THEN fd."LogSvcPrice"
             WHEN \'network\' THEN fd."NetworkPrice"
             ELSE 999.9 END as cost
FROM "costCompRawData" fd, vars v, bestIds bi WHERE bi.ops=fd.ops AND bi."DataSize"=fd."DataSize" AND fd.id=bi.bestId AND fd."Type"=bi."Type")
SELECT * FROM result;'))

costDistData$newOps <- factor(costDistData$ops, levels=rev(sort(unique(costDistData$ops))))
costDistData$newVar <- ordered(costDistData$var, levels=rev(c("prim","ebs","page","log","network","sec")))
costDistData$newType <- ordered(costDistData$Type, levels=archLevels)
plot_cost_comp <- ggplot(costDistData, aes(x=factor(newType), y=cost,fill=newVar)) +
    geom_col() +
    geom_text(aes(label=mylabel), size=1.8, vjust=1.5, color="white") +
    facet_grid(rows=vars(DataSize),cols=vars(ops), scales="free", labeller=labeller(ops=scientific_10, DataSize=datasetLabels)) +
    scale_x_discrete(name = "Architecture", label = archNames) +
    scale_fill_manual(values = componentColors, name = "Component") +
...
#        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title=element_text(size=18),
        strip.text.y = element_text(size = 14.0),
        strip.text.x = element_text(size = 14.0),
        legend.position = "right"
    )
ggsave(plot = plot_cost_comp, filename = "cost_comp.pdf", device = cairo_pdf, width = 10.7, height = 10) # 4.4 width for one
...
ggsave(plot = plot_cost_comp, filename = "availability.pdf", device = cairo_pdf, width = 6.0, height = 5) # 4.4 width for one
