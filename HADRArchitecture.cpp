...
unique_ptr<HADR> HADR::assemble(const Parameter& p2, Node n) {
   ...
   auto size = p.datasetSize + p.getRequiredAriesLogStorage();
...
Durability HADR::getDurability() const  {
  return Durability::calculateDurability(secondaries.getCount() + 1, primary.n.getAvailability().numericValue, parameter.datasetSize / 50_mib, 1 /*We stay durable if one node survives*/);
}
//--------------------------------------------------------------------------------
FailoverTime HADR::getFailoverTime() const {
  ...
  return Node::nodeSpinupTime +
    FailoverTime{parameter.datasetSize / throughput};
}
//--------------------------------------------------------------------------------
