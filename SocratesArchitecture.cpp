...
SocratesLike::SocratesLike(const Parameter& p, const Primary& prim, const Node& pageNode, unique_ptr<Ec2LogService> log)
  : Architecture{p, prim, ArchType::SocratesLike}, pageService{*Ec2PageService::assemble(parameter, primary, pageNode, Latency::deduce(parameter.requiredOpLatency, {{primary.probFirstCacheHit(), Memory::readLatency}, {primary.probSecondCacheHit(), InstanceStorage::readLatency}}))}, logService{*log} {
...
//--------------------------------------------------------------------------------
unique_ptr<SocratesLike> SocratesLike::assemble(const Parameter& p2, const Node& n, const Node& page, const Node& log) {
   auto p = p2;
   ...
   bool usesBufferPoolExtension = (n.name == "p4d.24") ? false : true;
   ...
   // For logging, we need to check if the EBS device of the log service can sustain the workload. The rest can scale arbitrarily
   auto logService = Ec2LogService::assemble(p, primary, log);
