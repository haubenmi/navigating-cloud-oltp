...
unique_ptr<InMemoryPageService> InMemoryPageService::assemble(const Parameter& p, Primary& prim) {
   if (prim.n.memory.getTotalSize() < p.datasetSize) return nullptr;
   return make_unique<InMemoryPageService>(p, prim);
}
//--------------------------------------------------------------------------------
unique_ptr<Ec2PageService> Ec2PageService::assemble(const Parameter& p, Primary& prim, Node pageNode, Latency targetLatency, bool useRbpex) {
  assert(pageNode.instanceStorage.devices > 0.0);

  double storageScale = (1.0 * p.datasetSize) / (pageNode.instanceStorage.getUsableSize() + (useRbpex ? pageNode.memory.getTotalSize() : 0));

  // A page server reads a log record from the network for every log record that gets applied
  double networkReadScale = p.requiredUpdateOps / pageNode.network.getReadLimit() / p.getLogRecordSize();
  ...
  auto memoryScaleForLatency = (p.datasetSize * minRequiredCacheHitRate) / pageNode.memory.getTotalSize();
  ...
  double iopsScale =  (requiredPageNodeGets * p.datasetSize) / (diskOps * p.datasetSize + requiredPageNodeGets * pageNode.memory.getTotalSize());
...
double Ec2PageService::getPageNodeCacheMiss() const {
  auto dataInPageNodeCache = min(parameter.datasetSize, static_cast<uint64_t>(pageNodeFraction * pageNode.memory.getTotalSize()));
  double pageNodeCacheMiss = (1.0 * (parameter.datasetSize - dataInPageNodeCache)) / parameter.datasetSize;
...
unique_ptr<EBSPageService> EBSPageService::assemble(const Parameter& parameter, Primary& prim, EBS::Type t) {
   auto size = parameter.datasetSize;
...
unique_ptr<CombinedPageServiceLog> CombinedPageServiceLog::assemble(const Parameter& p, Primary& prim, Node storageNode, Latency targetLatency) {
   double grossStorageSize = p.datasetSize * AuroraLike::dataReplication + p.getRequiredLogStorage() * AuroraLike::logReplication;
   ...
   Latency network = Latency::combine({{p.getSameAZRatio(), SameDatacenter::latency}, {p.getRemoteAZRatio(), SameRegion::latency}});
   double minRequiredCacheHitRate = Latency::getRatio(targetLatency - network.asAvg(), Memory::readLatency, InstanceStorage::readLatency);
   auto latencyScale = (p.datasetSize * minRequiredCacheHitRate) / storageNode.memory.getTotalSize();
   ...
   double iopsScale = (requiredPageNodeGets * p.datasetSize) / (diskOps * p.datasetSize + requiredPageNodeGets * memSize);
...
double CombinedPageServiceLog::getPageNodeCacheMiss() const {
   ...
   auto dataInPageNodeCache = min(p.datasetSize, static_cast<uint64_t>(fraction * n.memory.getTotalSize()));
   double pageNodeCacheMiss = (1.0 * (p.datasetSize - dataInPageNodeCache)) / p.datasetSize;
   assert(0 <= pageNodeCacheMiss && pageNodeCacheMiss <= 1.0);
   return pageNodeCacheMiss;
}
//--------------------------------------------------------------------------------
