...
struct Primary {
   ...
   std::string getDescription() const { return n.name + (usesBufferPoolExtension ? "-rbpex" : ""); }
   ...
   uint64_t getBufferCacheSize() const { return n.memory.getTotalSize() + (usesBufferPoolExtension ? n.instanceStorage.getUsableSize() : 0); }
   double probDirty() const { return p.requiredUpdateOps / (p.requiredUpdateOps + p.requiredLookupOps); }
   uint64_t dataInCache() const { return std::min(getBufferCacheSize(), p.datasetSize); }
   uint64_t dataNotInCache() const { return p.datasetSize - dataInCache(); }
   ...
   uint64_t dataInFirstCache() const { return std::min(n.memory.getTotalSize(), p.datasetSize); }
   uint64_t dataNotInFirstCache() const { return p.datasetSize - dataInFirstCache(); }
   ...
   uint64_t dataNotInSecondCache() const { return p.datasetSize - dataInSecondCache(); }
