...
struct SocratesLike : public Architecture {
   static constexpr uint64_t numPagesInChunk = Network::S3WriteTransferSize / InstanceStorage::MaxIOPSize;
   ...
   static constexpr Durability durability = std::min(Durability{EBS::io2_durability}, S3::durability);
   ...
   static std::unique_ptr<SocratesLike> assemble(const Parameter& p, const Node& n, const Node& page, const Node& log);
};
//--------------------------------------------------------------------------------
