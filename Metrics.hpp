...
struct PrimaryBufferCacheHitrate : public Metric {
  PrimaryBufferCacheHitrate() : Metric{"PrimCacheHitrate"} {}
...
struct TypeMetric : public Metric {
   ...
   std::partial_ordering compare(const Architecture& a, const Architecture& b) const override { return a.getTypeName() <=> b.getTypeName(); }
};
//--------------------------------------------------------------------------------
...
struct S3Price : public Metric {
   ...
   static Price get(const Architecture& a) {
      auto price = getS3StorageCost(a);
      price += S3::getPrice * a.getS3GETRate();
      price += S3::putPrice * a.getS3PUTRate();
      return price;
   }
...
struct NetworkPrice : public Metric {
   ...
   static Price get(const Architecture& a) {
      return (a.getInterAZTraffic() * 1.0 / 1_gib) * Network::interAZCost; // The traffic is per second
   }
...
struct TotalPrice : public Metric {
   ...
   static Price getPrice(const Architecture& a) {
      auto price = a.getPrimary().getPrice();
      price += a.getPrimary().getEBSPrice();
      price += a.getSecondaries().getPrice();
      // Secondaries always have the same EBS devices as the primary
      price += a.getSecondaries().getCount() * a.getPrimary().getEBSPrice();
      price += a.getPageService().getPrice();
      if (!a.getPageService().containsLogService()) {
         price += a.getLogService().getPrice();
      }
      price += NetworkPrice::get(a);
      price += S3Price::get(a);
      return price;
   }
...
struct OpLatencyMetric : public Metric {
   ...
   std::partial_ordering compare(const Architecture& a, const Architecture& b) const override { return a.getOpLatency().avg <=> b.getOpLatency().avg; }
};
//--------------------------------------------------------------------------------
